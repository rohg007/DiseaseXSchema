...
animalCaseRouter.route('/:animalCaseId')
.get((req,res,next)=>{
...
.post((req,res,next)=>{
    res.statusCode = 403;
    res.end('POST operation not supported on /animalCase/'+req.params.animalCaseId);
})
.put((req,res,next)=>{
    AnimalCases.findByIdAndUpdate(req.params.animalCaseId, {
        $set: req.body
    }, {new: true})
    .then((animalCase)=>{
        res.statusCode=200;
