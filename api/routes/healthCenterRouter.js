...
healthCenterRouter.route('/:healthCenterId')
.get((req,res,next)=>{
...
.post((req,res,next)=>{
    res.statusCode = 403;
    res.end('POST operation not supported on /healthCenters/'+req.params.healthCenterId);
})
.put((req,res,next)=>{
    HealthCenters.findByIdAndUpdate(req.params.healthCenterId, {
        $set: req.body
    }, {new: true})
    .then((healthCenter)=>{
        res.statusCode=200;
