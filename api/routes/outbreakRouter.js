...
.put((req,res,next)=>{
    res.statusCode = 403;
    res.end('PUT operation not supported on /outbreaks');
})
.delete((req,res,next)=>{
    Outbreaks.remove({})
    .then((resp)=>{
...
outbreakRouter.route('/:outbreakId')
.get((req,res,next)=>{
...
.post((req,res,next)=>{
    res.statusCode = 403;
    res.end('POST operation not supported on /outbreaks/'+req.params.outbreakId);
})
.put((req,res,next)=>{
    Outbreaks.findByIdAndUpdate(req.params.outbreakId, {
        $set: req.body
    }, {new: true})
    .then((outbreak)=>{
        res.statusCode=200;
