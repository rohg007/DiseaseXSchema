...
.put((req,res,next)=>{
    res.statusCode = 403;
    res.end('PUT operation not supported on /diseases');
})
.delete((req,res,next)=>{
    Diseases.remove({})
    .then((resp)=>{
...
.post((req,res,next)=>{
    res.statusCode = 403;
    res.end('POST operation not supported on /diseases/'+req.params.diseaseId);
})
.put((req,res,next)=>{
    Diseases.findByIdAndUpdate(req.params.diseaseId, {
        $set: req.body
    }, {new: true})
    .then((disease)=>{
        res.statusCode=200;
