...
.put((req,res,next)=>{
    res.statusCode = 403;
    res.end('PUT operation not supported on /animalOwners');
})
.delete((req,res,next)=>{
    AnimalOwner.remove({})
    .then((resp)=>{
...
animalOwnerRouter.route('/:animalOwnerId')
.get((req,res,next)=>{
...
.post((req,res,next)=>{
    res.statusCode = 403;
    res.end('POST operation not supported on /animalOwners/'+req.params.animalOwnerId);
})
.put((req,res,next)=>{
    AnimalOwner.findByIdAndUpdate(req.params.animalOwnerId, {
        $set: req.body
    }, {new: true})
    .then((animalOwner)=>{
        res.statusCode=200;
