...
.put((req,res,next)=>{
    res.statusCode = 403;
    res.end('PUT operation not supported on /livestocks');
})
.delete((req,res,next)=>{
    Livestocks.remove({})
    .then((resp)=>{
...
livestockRouter.route('/:livestockId')
.get((req,res,next)=>{
...
.post((req,res,next)=>{
    res.statusCode = 403;
    res.end('POST operation not supported on /livestocks/'+req.params.livestockId);
})
.put((req,res,next)=>{
    Livestocks.findByIdAndUpdate(req.params.livestockId, {
        $set: req.body
    }, {new: true})
    .then((livestock)=>{
        res.statusCode=200;
