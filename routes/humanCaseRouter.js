...
.put((req,res,next)=>{
    res.statusCode = 403;
    res.end('PUT operation not supported on /humanCases');
})
.delete((req,res,next)=>{
    HumanCases.remove({})
    .then((resp)=>{
...
humanCaseRouter.route('/:humanCaseId')
.get((req,res,next)=>{
...
.post((req,res,next)=>{
    res.statusCode = 403;
    res.end('POST operation not supported on /humanCase/'+req.params.humanCaseId);
})
.put((req,res,next)=>{
    HumanCases.findByIdAndUpdate(req.params.humanCaseId, {
        $set: req.body
    }, {new: true})
    .then((humanCase)=>{
        res.statusCode=200;
